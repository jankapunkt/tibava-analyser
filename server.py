...
from analyser.plugins import VideoPluginManager, AudioPluginManager

def init_plugins(config):
    data_dict = {}

    video_manager = VideoPluginManager(configs=config.get("image_text", []))
    video_manager.find()

    data_dict["video_manager"] = video_manager

    audio_manager = AudioPluginManager(configs=config.get("image_text", []))
    audio_manager.find()

    data_dict["audio_manager"] = audio_manager
...
class Commune(analyser_pb2_grpc.AnalyserServicer):
    def __init__(self, config):
        ...
        print(self.managers["video_manager"].plugins())
    ...
    def list_plugins(self, request, context):
        ...
        for plugin_name, plugin_class in self.managers["video_manager"].plugins().items():
            pluginInfo = reply.plugins.add()
            pluginInfo.name = plugin_name
            pluginInfo.type = "video"

        for plugin_name, plugin_class in self.managers["audio_manager"].plugins().items():
            pluginInfo = reply.plugins.add()
            pluginInfo.name = plugin_name
            pluginInfo.type = "audio"
