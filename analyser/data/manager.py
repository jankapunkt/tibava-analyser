...
class DataManager:
    ...
    @classmethod
    def _load_from_stream(cls, data_dir: str, data: Iterator[Any], data_id: str, save_meta=True) -> Data:
    ...
    def load_from_stream(self, data: Iterator[Any], data_id: str = None, save_meta=True) -> Data:
        if not data_id:
            data_id = generate_id()
        return self._load_from_stream(self.data_dir, data, data_id, save_meta)

    def dump_to_stream(self, data: Data):
        return data.dump_to_stream()

    def check(self, data_id: str, data_dir: str = None) -> Data:
        if not data_dir:
            data_dir = self.data_dir
        try:
            data = Data.load(data_dir=data_dir, id=data_id, load_blob=False)
            return data
        except:
            return None

    @classmethod
    def _load(self, data_dir: str, data_id: str) -> Data:
        data = Data.load(data_dir=data_dir, id=data_id, load_blob=False)
    ...
    def load(self, data_id: str) -> Data:
