...
class FaceClustering(
    ...
    def call(
        ...
        with inputs["embeddings"] as face_embeddings, inputs["faces"] as faces, inputs["bboxes"] as bboxes, inputs[
            "kpss"
        ] as kpss, inputs["images"] as images, data_manager.create_data(
            "FaceClusterData"
        ) as output_data, data_manager.create_data(
            "ImageEmbeddings"
        ) as mean_embeddings:
            ...
            zipped = sorted(
                zip(output_data.clusters, mean_embeddings.embeddings),
                key=lambda cluster: (len(cluster[0].face_refs)),
                reverse=True,
            )
