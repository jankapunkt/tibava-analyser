...
class VideoPluginManager(PluginManager):
    _feature_plugins = {}
    ...
    def find(self, path=os.path.join(os.path.abspath(os.path.dirname(__file__)), "video")):
        file_re = re.compile(r"(.+?)\.py$")
        for pl in os.listdir(path):
            match = re.match(file_re, pl)
            if match:
                a = importlib.import_module("analyser.plugins.video.{}".format(match.group(1)))
                # print(a)
                function_dir = dir(a)
