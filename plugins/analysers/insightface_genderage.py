...
class InsightfaceGenderAgeCalculator(AnalyserPlugin):
    ...
    def transform(self, img, center, output_size, scl):
        ...
        t4 = self.get_2d_matrix(translation=(output_size / 2,
                                                    output_size / 2))
        t = t2 @ t1
        t = t3 @ t
        t = t4 @ t
    ...
    def get_age(self, iterator, num_faces, parameters, callbacks):
        try:
            features = []
            ...
            for i, face in enumerate(iterator):
                ...
                gender = np.argmax(gender_age[:2]) # 0: female, 1: male
                age = int(np.round(gender_age[2] * 100)) # age in years
                #TODO convert age into age categories
                results = [gender, age]
            ...
            return {"gender_ages": ListData(data=features)}
                ...
                for frame in video_decoder:
                    ...
                    if t in bbox_dict:
                        for bbox in bbox_dict[t]:
                            yield {"frame": frame["frame"], "bbox": bbox}
